...
parser.add_argument('--loss_name', type=str, default='ce', required=False)
...
def train():
    ...
    for epoch in range(args.num_epoch):
        ...
        # save weights
        ckpt_path = os.path.join(ckpt_dir, '{}.newest.pt'.format(args.model_name))
        torch.save(model.state_dict(), ckpt_path)
        if not best_metric_dict: best_metric_dict = metric_dict
        if metric_dict['main'] >= best_metric_dict['main']:
            ckpt_path = os.path.join(ckpt_dir, '{}.best.pt'.format(args.model_name))
            torch.save(model.state_dict(), ckpt_path)
...
def evaluate():
    ...
    model.load_state_dict(state_dict, strict=False if args.model_name == 'googlenet' else True)
...
def inference():
    ...
    with open(os.path.join(args.result, '{}_inference.json'.format(args.model_name)), 'w') as f:
